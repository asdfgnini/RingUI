...
RingFrameless::RingFrameless(QQuickItem *parent) : QQuickItem{parent} {
    _fixSize = false;
    _appbar = nullptr;
    ...
    _topmost = false;
    _disabled = false;
    _isWindows11OrGreater = RingTools::getInstance()->isWindows11OrGreater();
}

RingFrameless::~RingFrameless() = default;

[[maybe_unused]] void RingFrameless::onDestruction() {
    QGuiApplication::instance()->removeNativeEventFilter(this);
}

void RingFrameless::componentComplete() {
    if (_disabled) {
        return;
    }
    int w = window()->width();
    ...
    connect(this, &RingFrameless::topmostChanged, this, [this] {
        _setWindowTopmost(topmost());
    });
    _setWindowTopmost(topmost());
}

[[maybe_unused]] bool RingFrameless::nativeEventFilter(const QByteArray &eventType, void *message, QT_NATIVE_EVENT_RESULT_TYPE *result) {
#ifdef Q_OS_WIN
...
bool RingFrameless::_isMaximized() {
    return window()->visibility() == QWindow::Maximized;
}

bool RingFrameless::_isFullScreen() {
    return window()->visibility() == QWindow::FullScreen;
}

void RingFrameless::_showSystemMenu(QPoint point) {
#ifdef Q_OS_WIN
    HWND hwnd = reinterpret_cast<HWND>(window()->winId());
    DWORD style = ::GetWindowLongPtr(hwnd, GWL_STYLE);
...
bool RingFrameless::_hitMaximizeButton() {
...
void RingFrameless::_setMaximizePressed(bool val) {
    _maximizeButton->setProperty("down", val);
}

void RingFrameless::_setMaximizeHovered(bool val) {
    _maximizeButton->setProperty("hover", val);
}

void RingFrameless::_updateCursor(int edges) {
    switch (edges) {
    case 0:
...
[[maybe_unused]] void RingFrameless::showFullScreen() {
    window()->showFullScreen();
}

void RingFrameless::showMaximized() {
#ifdef Q_OS_WIN
    HWND hwnd = reinterpret_cast<HWND>(window()->winId());
    ::ShowWindow(hwnd, 3);
#else
...
[[maybe_unused]] void RingFrameless::showMinimized() {
#ifdef Q_OS_WIN
    HWND hwnd = reinterpret_cast<HWND>(window()->winId());
    ::ShowWindow(hwnd, 2);
#else
...
void RingFrameless::showNormal() {
    window()->setVisibility(QQuickWindow::Windowed);
}

void RingFrameless::setHitTestVisible(QQuickItem *val) {
    if (!_hitTestList.contains(val)) {
        _hitTestList.append(val);
    }
}

void RingFrameless::_setWindowTopmost(bool topmost) {
#ifdef Q_OS_WIN
    HWND hwnd = reinterpret_cast<HWND>(window()->winId());
    if (topmost) {
...
bool RingFrameless::eventFilter(QObject *obj, QEvent *ev) {
#ifndef Q_OS_WIN
    switch (ev->type()) {
